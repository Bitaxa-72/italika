...
const subscribeOpenButtons = document.querySelectorAll('[data-subscribe-open]');
const subscribeCloseButtons = subscribeModal?.querySelectorAll('[data-subscribe-close]');
const subscribeEmailInput = subscribeModal?.querySelector('.subscribe-modal__input');
...
const setSubscribeModalState = (isOpen) => {
  ...
  subscribeModal.setAttribute('aria-hidden', String(!isOpen));
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && subscribeModal?.classList.contains('is-open')) {
    setSubscribeModalState(false);
  }
});
